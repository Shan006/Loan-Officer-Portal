const PredefinedForm = [
  {
    title: "DEFAULT NEED LIST",
    data: [
      {
        data: "Copies of Government Issued Identification",
        status: true,
      },
      {
        data: "Borrower Authorization",
        status: false,
      },
      {
        data: "1099 or W2 Forms (Last 2 Years)",
        status: true,
      },
      {
        data: "1040 Federal Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Business Tax Returns (Last 2 Years)",
        status: false,
      },
      {
        data: "Paystubs (Last 30 Days)",
        status: true,
      },
      {
        data: "Most Recent Bank Statements (Last 2 Months)",
        status: true,
      },
      {
        data: "Mortgage Statement/Coupons (For All Loans)",
        status: true,
      },
      {
        data: "Credit Report",
        status: false,
      },
      {
        data: "Property Tax Bill",
        status: false,
      },
      {
        data: "Appraisal Credit Authorization",
        status: false,
      },
      {
        data: "Retirement/Investment Account Statements",
        status: false,
      },
      {
        data: "Profit and Loss Statements (YTD)",
        status: false,
      },
      {
        data: "Divorce Decree",
        status: false,
      },
      {
        data: "Social Security Award Letter",
        status: false,
      },
      {
        data: "Homeowners Insurance",
        status: false,
      },
      {
        data: "Gift Letter (If Applicable)",
        status: false,
      },
      {
        data: "Cancelled Rent Checks or Mortgage Checks",
        status: false,
      },
      {
        data: "Bankruptcy Documents",
        status: false,
      },
      {
        data: "VA DD-214",
        status: false,
      },
      {
        data: "Rental History (1 Year)",
        status: false,
      },
      {
        data: "CPA Letter (If Applicable)",
        status: false,
      },
      {
        data: "Business License (If Applicable)",
        status: false,
      },
      {
        data: "Executed Purchase Agreement",
        status: false,
      },
      {
        data: "1003 Loan Application",
        status: false,
      },
      {
        data: "VA Nearest Living Relative Form",
        status: false,
      },
      {
        data: "Escrow Instructions (if applicable)",
        status: false,
      },
      {
        data: "Source of Down Payment Funds and Earnest Money Deposit",
        status: false,
      },
      {
        data: "Business Tax Returns",
        status: false,
      },
      {
        data: "Most Recent Business Tax Returns (Last 1 Year)",
        status: false,
      },
      {
        data: "Lease Agreement(s)",
        status: false,
      },
      {
        data: "VA COE (Certification Of Eligibility)",
        status: false,
      },
      {
        data: "HOA Statement (If Applicable)",
        status: false,
      },
      {
        data: "Evidence of Insurance",
        status: false,
      },
      {
        data: "Pension Statement",
        status: false,
      },
      {
        data: "K-1 Statements",
        status: false,
      },
      {
        data: "Private Money Loan App",
        status: false,
      },
      {
        data: "Final Closing Statements",
        status: false,
      },
      {
        data: "Asset Statements",
        status: false,
      },
      {
        data: "Bank Statements (1 Year)",
        status: false,
      },
      {
        data: "Letter of Explanation for Any and All Large Deposits",
        status: false,
      },
      {
        data: "Trust Documentation (If Applicable)",
        status: false,
      },
      {
        data: "Loan Modification Agreement",
        status: false,
      },
      {
        data: "2018 W2s",
        status: false,
      },
      {
        data: "Wells Fargo Bank Statement #0595",
        status: false,
      },
      {
        data: "Nov Bank statements #0595",
        status: false,
      },
    ],
  },
  {
    title: "PURCHASE DOCUMENTS FOR W-2 AND 1099 EMPLOYEES",
    data: [
      {
        data: "Copies of Government Issued Identification",
        status: true,
      },
      {
        data: "Borrower Authorization",
        status: false,
      },
      {
        data: "1099 or W2 Forms (Last 2 Years)",
        status: true,
      },
      {
        data: "1040 Federal Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Business Tax Returns (Last 2 Years)",
        status: false,
      },
      {
        data: "Paystubs (Last 30 Days)",
        status: true,
      },
      {
        data: "Most Recent Bank Statements (Last 2 Months)",
        status: true,
      },
      {
        data: "Mortgage Statement/Coupons (For All Loans)",
        status: false,
      },
      {
        data: "Credit Report",
        status: false,
      },
      {
        data: "Property Tax Bill",
        status: false,
      },
      {
        data: "Appraisal Credit Authorization",
        status: true,
      },
      {
        data: "Retirement/Investment Account Statements",
        status: true,
      },
      {
        data: "Profit and Loss Statements (YTD)",
        status: false,
      },
      {
        data: "Divorce Decree",
        status: true,
      },
      {
        data: "Social Security Award Letter",
        status: false,
      },
      {
        data: "Homeowners Insurance",
        status: true,
      },
      {
        data: "Gift Letter (If Applicable)",
        status: true,
      },
      {
        data: "Cancelled Rent Checks or Mortgage Checks",
        status: true,
      },
      {
        data: "Bankruptcy Documents",
        status: true,
      },
      {
        data: "VA DD-214",
        status: false,
      },
      {
        data: "Rental History (1 Year)",
        status: false,
      },
      {
        data: "CPA Letter (If Applicable)",
        status: false,
      },
      {
        data: "Business License (If Applicable)",
        status: false,
      },
      {
        data: "Executed Purchase Agreement",
        status: false,
      },
      {
        data: "1003 Loan Application",
        status: false,
      },
      {
        data: "VA Nearest Living Relative Form",
        status: false,
      },
      {
        data: "Escrow Instructions (if applicable)",
        status: false,
      },
      {
        data: "Source of Down Payment Funds and Earnest Money Deposit",
        status: false,
      },
      {
        data: "Business Tax Returns",
        status: false,
      },
      {
        data: "Most Recent Business Tax Returns (Last 1 Year)",
        status: false,
      },
      {
        data: "Lease Agreement(s)",
        status: false,
      },
      {
        data: "VA COE (Certification Of Eligibility)",
        status: false,
      },
      {
        data: "HOA Statement (If Applicable)",
        status: false,
      },
      {
        data: "Evidence of Insurance",
        status: true,
      },
      {
        data: "Pension Statement",
        status: false,
      },
      {
        data: "K-1 Statements",
        status: false,
      },
      {
        data: "Private Money Loan App",
        status: false,
      },
      {
        data: "Final Closing Statements",
        status: false,
      },
      {
        data: "Asset Statements",
        status: true,
      },
      {
        data: "Bank Statements (1 Year)",
        status: false,
      },
      {
        data: "Letter of Explanation for Any and All Large Deposits",
        status: true,
      },
      {
        data: "Trust Documentation (If Applicable)",
        status: false,
      },
      {
        data: "Loan Modification Agreement",
        status: false,
      },
      {
        data: "2018 W2s",
        status: false,
      },
      {
        data: "Wells Fargo Bank Statement #0595",
        status: false,
      },
      {
        data: "Nov Bank statements #0595",
        status: false,
      },
    ],
  },
  {
    title: "PURCHASE DOCUMENTS FOR W-2 AND 1099 EMPLOYEES-VA LOANS ",
    data: [
      {
        data: "Copies of Government Issued Identification",
        status: true,
      },
      {
        data: "Borrower Authorization",
        status: false,
      },
      {
        data: "1099 or W2 Forms (Last 2 Years)",
        status: true,
      },
      {
        data: "1040 Federal Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Business Tax Returns (Last 2 Years)",
        status: false,
      },
      {
        data: "Paystubs (Last 30 Days)",
        status: true,
      },
      {
        data: "Most Recent Bank Statements (Last 2 Months)",
        status: true,
      },
      {
        data: "Mortgage Statement/Coupons (For All Loans)",
        status: true,
      },
      {
        data: "Credit Report",
        status: false,
      },
      {
        data: "Property Tax Bill",
        status: false,
      },
      {
        data: "Appraisal Credit Authorization",
        status: true,
      },
      {
        data: "Retirement/Investment Account Statements",
        status: true,
      },
      {
        data: "Profit and Loss Statements (YTD)",
        status: false,
      },
      {
        data: "Divorce Decree",
        status: true,
      },
      {
        data: "Social Security Award Letter",
        status: true,
      },
      {
        data: "Homeowners Insurance",
        status: true,
      },
      {
        data: "Gift Letter (If Applicable)",
        status: true,
      },
      {
        data: "Cancelled Rent Checks or Mortgage Checks",
        status: false,
      },
      {
        data: "Bankruptcy Documents",
        status: true,
      },
      {
        data: "VA DD-214",
        status: true,
      },
      {
        data: "Rental History (1 Year)",
        status: true,
      },
      {
        data: "CPA Letter (If Applicable)",
        status: false,
      },
      {
        data: "Business License (If Applicable)",
        status: false,
      },
      {
        data: "Executed Purchase Agreement",
        status: false,
      },
      {
        data: "1003 Loan Application",
        status: true,
      },
      {
        data: "VA Nearest Living Relative Form",
        status: true,
      },
      {
        data: "Escrow Instructions (if applicable)",
        status: false,
      },
      {
        data: "Source of Down Payment Funds and Earnest Money Deposit",
        status: true,
      },
      {
        data: "Business Tax Returns",
        status: false,
      },
      {
        data: "Most Recent Business Tax Returns (Last 1 Year)",
        status: false,
      },
      {
        data: "Lease Agreement(s)",
        status: false,
      },
      {
        data: "VA COE (Certification Of Eligibility)",
        status: true,
      },
      {
        data: "HOA Statement (If Applicable)",
        status: false,
      },
      {
        data: "Evidence of Insurance",
        status: false,
      },
      {
        data: "Pension Statement",
        status: true,
      },
      {
        data: "K-1 Statements",
        status: false,
      },
      {
        data: "Private Money Loan App",
        status: false,
      },
      {
        data: "Final Closing Statements",
        status: false,
      },
      {
        data: "Asset Statements",
        status: true,
      },
      {
        data: "Bank Statements (1 Year)",
        status: false,
      },
      {
        data: "Letter of Explanation for Any and All Large Deposits",
        status: true,
      },
      {
        data: "Trust Documentation (If Applicable)",
        status: false,
      },
      {
        data: "Loan Modification Agreement",
        status: false,
      },
      {
        data: "2018 W2s",
        status: false,
      },
      {
        data: "Wells Fargo Bank Statement #0595",
        status: false,
      },
      {
        data: "Nov Bank statements #0595",
        status: false,
      },
    ],
  },
  {
    title: "PURCHASE DOCUMENTS FOR SELF-EMPLOYED BROROWERS",
    data: [
      {
        data: "Copies of Government Issued Identification",
        status: true,
      },
      {
        data: "Borrower Authorization",
        status: false,
      },
      {
        data: "1099 or W2 Forms (Last 2 Years)",
        status: true,
      },
      {
        data: "1040 Federal Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Business Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Paystubs (Last 30 Days)",
        status: true,
      },
      {
        data: "Most Recent Bank Statements (Last 2 Months)",
        status: true,
      },
      {
        data: "Mortgage Statement/Coupons (For All Loans)",
        status: true,
      },
      {
        data: "Credit Report",
        status: false,
      },
      {
        data: "Property Tax Bill",
        status: false,
      },
      {
        data: "Appraisal Credit Authorization",
        status: true,
      },
      {
        data: "Retirement/Investment Account Statements",
        status: true,
      },
      {
        data: "Profit and Loss Statements (YTD)",
        status: true,
      },
      {
        data: "Divorce Decree",
        status: true,
      },
      {
        data: "Social Security Award Letter",
        status: false,
      },
      {
        data: "Homeowners Insurance",
        status: true,
      },
      {
        data: "Gift Letter (If Applicable)",
        status: true,
      },
      {
        data: "Cancelled Rent Checks or Mortgage Checks",
        status: false,
      },
      {
        data: "Bankruptcy Documents",
        status: false,
      },
      {
        data: "VA DD-214",
        status: false,
      },
      {
        data: "Rental History (1 Year)",
        status: true,
      },
      {
        data: "CPA Letter (If Applicable)",
        status: true,
      },
      {
        data: "Business License (If Applicable)",
        status: true,
      },
      {
        data: "Executed Purchase Agreement",
        status: true,
      },
      {
        data: "1003 Loan Application",
        status: true,
      },
      {
        data: "VA Nearest Living Relative Form",
        status: false,
      },
      {
        data: "Escrow Instructions (if applicable)",
        status: false,
      },
      {
        data: "Source of Down Payment Funds and Earnest Money Deposit",
        status: false,
      },
      {
        data: "Business Tax Returns",
        status: true,
      },
      {
        data: "Most Recent Business Tax Returns (Last 1 Year)",
        status: false,
      },
      {
        data: "Lease Agreement(s)",
        status: false,
      },
      {
        data: "VA COE (Certification Of Eligibility)",
        status: false,
      },
      {
        data: "HOA Statement (If Applicable)",
        status: false,
      },
      {
        data: "Evidence of Insurance",
        status: false,
      },
      {
        data: "Pension Statement",
        status: false,
      },
      {
        data: "K-1 Statements",
        status: true,
      },
      {
        data: "Private Money Loan App",
        status: false,
      },
      {
        data: "Final Closing Statements",
        status: false,
      },
      {
        data: "Asset Statements",
        status: true,
      },
      {
        data: "Bank Statements (1 Year)",
        status: false,
      },
      {
        data: "Letter of Explanation for Any and All Large Deposits",
        status: true,
      },
      {
        data: "Trust Documentation (If Applicable)",
        status: false,
      },
      {
        data: "Loan Modification Agreement",
        status: false,
      },
      {
        data: "2018 W2s",
        status: false,
      },
      {
        data: "Wells Fargo Bank Statement #0595",
        status: false,
      },
      {
        data: "Nov Bank statements #0595",
        status: false,
      },
    ],
  },
  {
    title: "REFINANCE DOCUMENTS FOR W-2 AND 1099 EMPLOYEES ",
    data: [
      {
        data: "Copies of Government Issued Identification",
        status: true,
      },
      {
        data: "Borrower Authorization",
        status: false,
      },
      {
        data: "1099 or W2 Forms (Last 2 Years)",
        status: true,
      },
      {
        data: "1040 Federal Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Business Tax Returns (Last 2 Years)",
        status: false,
      },
      {
        data: "Paystubs (Last 30 Days)",
        status: true,
      },
      {
        data: "Most Recent Bank Statements (Last 2 Months)",
        status: true,
      },
      {
        data: "Mortgage Statement/Coupons (For All Loans)",
        status: true,
      },
      {
        data: "Credit Report",
        status: false,
      },
      {
        data: "Property Tax Bill",
        status: true,
      },
      {
        data: "Appraisal Credit Authorization",
        status: true,
      },
      {
        data: "Retirement/Investment Account Statements",
        status: true,
      },
      {
        data: "Profit and Loss Statements (YTD)",
        status: false,
      },
      {
        data: "Divorce Decree",
        status: false,
      },
      {
        data: "Social Security Award Letter",
        status: false,
      },
      {
        data: "Homeowners Insurance",
        status: true,
      },
      {
        data: "Gift Letter (If Applicable)",
        status: false,
      },
      {
        data: "Cancelled Rent Checks or Mortgage Checks",
        status: true,
      },
      {
        data: "Bankruptcy Documents",
        status: false,
      },
      {
        data: "VA DD-214",
        status: false,
      },
      {
        data: "Rental History (1 Year)",
        status: false,
      },
      {
        data: "CPA Letter (If Applicable)",
        status: false,
      },
      {
        data: "Business License (If Applicable)",
        status: false,
      },
      {
        data: "Executed Purchase Agreement",
        status: false,
      },
      {
        data: "1003 Loan Application",
        status: true,
      },
      {
        data: "VA Nearest Living Relative Form",
        status: false,
      },
      {
        data: "Escrow Instructions (if applicable)",
        status: false,
      },
      {
        data: "Source of Down Payment Funds and Earnest Money Deposit",
        status: false,
      },
      {
        data: "Business Tax Returns",
        status: false,
      },
      {
        data: "Most Recent Business Tax Returns (Last 1 Year)",
        status: false,
      },
      {
        data: "Lease Agreement(s)",
        status: false,
      },
      {
        data: "VA COE (Certification Of Eligibility)",
        status: false,
      },
      {
        data: "HOA Statement (If Applicable)",
        status: false,
      },
      {
        data: "Evidence of Insurance",
        status: false,
      },
      {
        data: "Pension Statement",
        status: false,
      },
      {
        data: "K-1 Statements",
        status: true,
      },
      {
        data: "Private Money Loan App",
        status: false,
      },
      {
        data: "Final Closing Statements",
        status: false,
      },
      {
        data: "Asset Statements",
        status: true,
      },
      {
        data: "Bank Statements (1 Year)",
        status: false,
      },
      {
        data: "Letter of Explanation for Any and All Large Deposits",
        status: true,
      },
      {
        data: "Trust Documentation (If Applicable)",
        status: false,
      },
      {
        data: "Loan Modification Agreement",
        status: false,
      },
      {
        data: "2018 W2s",
        status: false,
      },
      {
        data: "Wells Fargo Bank Statement #0595",
        status: false,
      },
      {
        data: "Nov Bank statements #0595",
        status: false,
      },
    ],
  },
  {
    title: "REFINANCE DOCUMENTS FOR SELF EMPLOYED BORROWERS ",
    data: [
      {
        data: "Copies of Government Issued Identification",
        status: true,
      },
      {
        data: "Borrower Authorization",
        status: false,
      },
      {
        data: "1099 or W2 Forms (Last 2 Years)",
        status: true,
      },
      {
        data: "1040 Federal Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Business Tax Returns (Last 2 Years)",
        status: true,
      },
      {
        data: "Paystubs (Last 30 Days)",
        status: true,
      },
      {
        data: "Most Recent Bank Statements (Last 2 Months)",
        status: true,
      },
      {
        data: "Mortgage Statement/Coupons (For All Loans)",
        status: true,
      },
      {
        data: "Credit Report",
        status: false,
      },
      {
        data: "Property Tax Bill",
        status: true,
      },
      {
        data: "Appraisal Credit Authorization",
        status: false,
      },
      {
        data: "Retirement/Investment Account Statements",
        status: true,
      },
      {
        data: "Profit and Loss Statements (YTD)",
        status: true,
      },
      {
        data: "Divorce Decree",
        status: false,
      },
      {
        data: "Social Security Award Letter",
        status: false,
      },
      {
        data: "Homeowners Insurance",
        status: true,
      },
      {
        data: "Gift Letter (If Applicable)",
        status: false,
      },
      {
        data: "Cancelled Rent Checks or Mortgage Checks",
        status: true,
      },
      {
        data: "Bankruptcy Documents",
        status: false,
      },
      {
        data: "VA DD-214",
        status: false,
      },
      {
        data: "Rental History (1 Year)",
        status: false,
      },
      {
        data: "CPA Letter (If Applicable)",
        status: false,
      },
      {
        data: "Business License (If Applicable)",
        status: false,
      },
      {
        data: "Executed Purchase Agreement",
        status: false,
      },
      {
        data: "1003 Loan Application",
        status: false,
      },
      {
        data: "VA Nearest Living Relative Form",
        status: false,
      },
      {
        data: "Escrow Instructions (if applicable)",
        status: false,
      },
      {
        data: "Source of Down Payment Funds and Earnest Money Deposit",
        status: false,
      },
      {
        data: "Business Tax Returns",
        status: false,
      },
      {
        data: "Most Recent Business Tax Returns (Last 1 Year)",
        status: false,
      },
      {
        data: "Lease Agreement(s)",
        status: false,
      },
      {
        data: "VA COE (Certification Of Eligibility)",
        status: false,
      },
      {
        data: "HOA Statement (If Applicable)",
        status: true,
      },
      {
        data: "Evidence of Insurance",
        status: false,
      },
      {
        data: "Pension Statement",
        status: false,
      },
      {
        data: "K-1 Statements",
        status: true,
      },
      {
        data: "Private Money Loan App",
        status: false,
      },
      {
        data: "Final Closing Statements",
        status: false,
      },
      {
        data: "Asset Statements",
        status: false,
      },
      {
        data: "Bank Statements (1 Year)",
        status: false,
      },
      {
        data: "Letter of Explanation for Any and All Large Deposits",
        status: true,
      },
      {
        data: "Trust Documentation (If Applicable)",
        status: false,
      },
      {
        data: "Loan Modification Agreement",
        status: false,
      },
      {
        data: "2018 W2s",
        status: false,
      },
      {
        data: "Wells Fargo Bank Statement #0595",
        status: false,
      },
      {
        data: "Nov Bank statements #0595",
        status: false,
      },
    ],
  },
];
export default PredefinedForm;
